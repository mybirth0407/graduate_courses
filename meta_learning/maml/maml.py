...
class MAML:
    ...
    def _inner_loop(self, theta, support_data):
        ...
        # adaptation
        for images, labels in support_data.take(
            self.train_data.n_support // self.train_data.n_way
        ):
            # pre-adaptation
            logits = self.model(images, training=False)
            loss_value = loss_fn(labels, logits)
            metrics_fn.update_state(labels, logits)
            ...
            for _ in range(self._num_inner_steps):
                ...
                inner_gradients = inner_tape.gradient(loss_value, self.model.trainable_weights)
                opt_fn.apply_gradients(zip(inner_gradients, self.model.trainable_weights))
                metrics_fn.update_state(labels, logits)
                accuracies.append(metrics_fn.result())
    ...
    def _outer_loop(self, task_batch, train=None):
        ...
        # Use keras.losses.SparseCategoricalCrossentropy to compute 
        # classification losses
        loss_fn = tf.keras.losses.SparseCategoricalCrossentropy()

        # .take() method for BatchDataset
        for task in task_batch:
            support, query = task

            with tf.GradientTape() as outer_tape:
                phi, inner_accuracies = self._inner_loop(theta, support)
                accuracies_support_batch.append(inner_accuracies)

                for images, labels in query.take(
                    self.train_data.n_query // self.train_data.n_way
                ):
                    # print(loss_value)
                    logits = self.model(images, training=train)
                    loss_value = loss_fn(labels, logits)
                    outer_loss_batch.append(loss_value)

                    metrics_fn.update_state(labels, logits)
                    accuracy_query_batch.append(metrics_fn.result())

                outer_gradients = outer_tape.gradient(tf.reduce_mean(outer_loss_batch), self.model.trainable_weights)
                self._optimizer.apply_gradients(zip(outer_gradients, theta))
...
def main(args):
    ...
    if log_dir is None: log_dir = os.path.join(os.path.abspath('.'), 'p1_log2')
