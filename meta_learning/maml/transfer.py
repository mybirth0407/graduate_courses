...
class TransferLearning:
    ...
    def _transfer_learn(self, task_batch):
        ...
        model_layers = []
        model_layers.append(feature_extractor)
        new_layer = layers.Dense(5, activation='softmax', name='Classification')
        model_layers.append(new_layer)

        new_model = keras.Sequential(model_layers)
        new_model.compile(
            optimizer=opt_fn,
            loss=loss_fn,
            metrics=metrics_fn
        ...
        for task in task_batch:
            ...
            adpt_metrics = new_model.fit(support, epochs=self._num_inner_steps, verbose=0)
            accuracies_support_batch.append(adpt_metrics.history['Accuracy'])

            eval_metrics = new_model.evaluate(query)
            outer_loss_batch.append(eval_metrics[0])
            accuracy_query_batch.append(eval_metrics[1])
