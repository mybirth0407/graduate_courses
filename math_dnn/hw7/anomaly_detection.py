...
for images, _ in test_loader:
    ...
    incorrect += torch.sum(detection).item()

print(f'anomaly count: {incorrect}')
print(f'type I error: {incorrect / len(test_dataset)}')
...
for images, _ in anomaly_loader:
    ...
    error = error.mean(1)
    detection = torch.where(error > threshold, 1, 0)
    correct += torch.sum(detection).item()

print(f'anomaly count: {correct}')
print(f'type II error: {1 - (correct / len(anomaly_dataset))}')