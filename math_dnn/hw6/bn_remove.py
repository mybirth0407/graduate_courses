...
def main():
    ...
    model.load_state_dict(
        torch.load("./smallNetSaved", map_location=torch.device('cpu'))
    )
    model_test = smallNetTest()

    # Initialize weights of model without BN
    conv1_bn_beta, conv1_bn_gamma = (
         model.conv1[1].bias, model.conv1[1].weight
    )
    conv1_bn_mean, conv1_bn_var = (
        model.conv1[1].running_mean, model.conv1[1].running_var
    )
    conv2_bn_beta, conv2_bn_gamma = (
        model.conv2[1].bias, model.conv2[1].weight
    )
    conv2_bn_mean, conv2_bn_var = (
        model.conv2[1].running_mean, model.conv2[1].running_var
    )

    fc1_bn_beta, fc1_bn_gamma = (
        model.fc1[1].bias, model.fc1[1].weight
    )
    fc1_bn_mean, fc1_bn_var = (
        model.fc1[1].running_mean, model.fc1[1].running_var
    )
    eps = 1e-05

    # Initialize the following parameters
    model_test.conv1[0].weight.data = (
        (model.conv1[0].weight.data * conv1_bn_gamma.reshape(-1,1,1,1))
        / torch.sqrt(conv1_bn_var.reshape(-1,1,1,1) + eps)
    )

    model_test.conv1[0].bias.data = (
        conv1_bn_gamma * (model.conv1[0].bias.data - conv1_bn_mean)
        / torch.sqrt(conv1_bn_var + eps)
    ) + conv1_bn_beta

    model_test.conv2[0].weight.data =  (
        (model.conv2[0].weight.data * conv2_bn_gamma.reshape(-1,1,1,1))
        / torch.sqrt(conv2_bn_var.reshape(-1,1,1,1) + eps)
    )
    model_test.conv2[0].bias.data = (
        conv2_bn_gamma * (model.conv2[0].bias.data - conv2_bn_mean)
        / torch.sqrt(conv2_bn_var + eps)
    ) + conv2_bn_beta

    model_test.fc1[0].weight.data = (
        (model.fc1[0].weight.data * fc1_bn_gamma.reshape(-1,1))
        / torch.sqrt(fc1_bn_var.reshape(-1,1) + eps)
    )
    model_test.fc1[0].bias.data = (
        fc1_bn_gamma * (model.fc1[0].bias.data - fc1_bn_mean)
